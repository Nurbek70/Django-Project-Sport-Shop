...
Shopify.onCartUpdate = function(cart) {
  Shopify.cartUpdateInfo(cart, '.cart-items .single-cart-item-loop');
};
...
Shopify.cartPopap = function(variant_id) {
  	...
  	var mc_img = jQuery('.'+variant_id+':first .mini_img').attr("src");
  	jQuery('.productmsg').text(''+title+'');  
  	jQuery('.rhs_img').attr("src", ''+mc_img+'');
  	jQuery('#modalAddToCart').modal("toggle");
}
Shopify.cartPopapForm = function(variant_id) {
  	var title = jQuery('.product-info__title').text();
  	var mc_img = jQuery('.mini_img').attr("src");
	jQuery('.productmsg').text(''+title+'');  
  	jQuery('.rhs_img').attr("src", ''+mc_img+'');
	jQuery('#modalAddToCart').modal("toggle");
}
    ...
    if (cart_cell.length) {
      ...
      jQuery.each(cart, function(key, value) {
        if (key === 'items') {
          ...
          if (value.length) {
            jQuery(".cart-items, .single-cart-item-loop").css({"display": "block"});
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<div class="cart-float-single-item d-flex"><span class="remove-item"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')"><i class="fa fa-times"></i></a></span><div class="cart-float-single-item-image"><a href="' + item.url + '"><img src="' + item.image + '" class="img-fluid" alt="{{ item.img.alt }}"></a></div><div class="cart-float-single-item-desc"><p class="product-title"> <a href="' + item.url + '">' + item.title + '</a></p><p class="price"><span class="count">' + item.quantity + '</span> x ' + Shopify.formatMoney(item.price, formatMoney) + '</p></div></div>').appendTo(table);
            });
          }
          else {
            jQuery(".cart-items, .single-cart-item-loop").css({"display": "none"});
